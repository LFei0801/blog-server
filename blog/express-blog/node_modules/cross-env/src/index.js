...
const constValueConvert = require('./constiable')
...
function crossEnv(args, options = {}) {
  ...
  const env = getEnvconsts(envSetters)
  if (command) {
    const proc = spawn(
...
function getEnvconsts(envSetters) {
  const envconsts = {...process.env}
  if (process.env.APPDATA) {
    envconsts.APPDATA = process.env.APPDATA
  }
  Object.keys(envSetters).forEach(constName => {
    envconsts[constName] = constValueConvert(envSetters[constName], constName)
  })
  return envconsts
}
